...
def zapisz_statystyki_pliku(filename, lilosc_uruchomien, liczba_wyrazow, liczba_znakow):
    stats_filename = f'report_' + filename
    try:
       with open(stats_filename, 'w+') as stats_plik:
           stats_plik.seek(0)
           stats_plik.write(f"Ilość uruchomień: "+lilosc_uruchomien
                      +"\nLiczba wyrazów: "+liczba_wyrazow
...
def pobierz_statystyki_pliku(filename):
    try:
        with open(filename, 'r+') as plik:
            ...
            zapisz_statystyki_pliku(filename, ilosc_uruchomien, liczba_wyrazow, liczba_znakow)
